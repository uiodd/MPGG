...
class Transformer_Based_Model(nn.Module):
    def __init__(self, dataset, temp, D_text, D_visual, D_audio, n_head,
        ...
        # 聚类级对比门控已集成到enhanced_gated_attention中
    ...
    def forward(self, textf, visuf, acouf, u_mask, qmask, dia_len, emotion_labels=None):
        ...
        # Multimodal-level Gated Fusion (保留原有融合)
        ...
        # 【修改】使用增强后的特征进行最终分类
        # 将原始单模态Transformer输出(早期融合)也考虑进来，与后期融合特征进行拼接或融合
        # t_t_transformer_out, a_a_transformer_out, v_v_transformer_out 是最早期的模态内自注意力特征
        # 我们将它们降维后与 enhanced_all_out 融合
        
        # 降维处理，将三个模态的早期特征融合并降维到 hidden_dim
        early_fusion = t_t_transformer_out + a_a_transformer_out + v_v_transformer_out
        
        # 最终融合：结合 早期特征(early_fusion) + 后期门控特征(enhanced_all_out)
        ...
        # 【修改】返回特征表示用于情感约束损失，包括增强后的特征和聚类对比损失
        proto_loss = None
        if emotion_labels is not None:
            # 使用投影头将16维特征映射到128维
